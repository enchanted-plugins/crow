...
3. You're about to declare a verdict (SHIP, DONE, "looks good", "trust restored") on your own output.
...
3. Name one piece of concrete evidence against — a precedent entry,
   a known failure code, a trust score, a side effect, a missing case.
...
| "Just waive the low-trust flag, it's fine" | "OK, waiving." | "V6 EMA reads real overrides as evidence — silent waivers poison the prior for similar future changes. I can override with a logged reason, or pause and answer the adversarial questions. Which?" |
| *(your own prior claim)* "no test covers this path" | "Right, adding one." | "I framed that too narrowly — the integration suite hits this path indirectly. Re-checking before adding a duplicate." |
...
2. Cite the evidence (memory entry, precedent log, trust score, file content, observed test).
